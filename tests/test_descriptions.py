...
# @lat: [[descriptions#Tests#Main wiring and legacy cleanup]]
def test_main_wires_takeout_enqueue_and_legacy_cleanup(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """main deletes stale JSON files, enqueues takeout ids, and calls fetch_descriptions."""
    db_path = tmp_path / "descriptions.sqlite"
    legacy_json = tmp_path / "descriptions.json"
    legacy_tmp = tmp_path / "descriptions.json.tmp"
    legacy_json.write_text("{}", encoding="utf-8")
    legacy_tmp.write_text("{}", encoding="utf-8")

    monkeypatch.setattr(descriptions.config, "DESCRIPTIONS_DB_PATH", db_path)
    monkeypatch.setattr(descriptions.config, "DESCRIPTIONS_CACHE_PATH", legacy_json)
    monkeypatch.setattr(descriptions.takeout, "load_video_ids", lambda: ["x", "y", "y"])

    called: list[Path | None] = []

    def fake_fetch(db: Path | None = None) -> None:
        called.append(db)

    monkeypatch.setattr(descriptions, "fetch_descriptions", fake_fetch)
    ...
    con = sqlite3.connect(db_path)
    try:
        rows = con.execute("SELECT video_id, status FROM descriptions ORDER BY video_id").fetchall()
    finally:
        con.close()
    assert rows == [("x", "pending"), ("y", "pending")]
